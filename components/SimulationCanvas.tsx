...
export const SimulationCanvas: React.FC<SimulationCanvasProps> = ({
    ...
    useEffect(() => {
        const img = new Image();
        img.src = './assets/images/rainforest.png';
        img.onload = () => {
    ...
    const engineRef = useRef({
        ...
        shootingStars: [] as ShootingStar[], // Gameplay background shooting stars
        nextShootingStar: 0, // timestamp for next spawn
    ...
    useEffect(() => {
        if (gameState === GameState.RUNNING && engineRef.current.balls.length === 0) {
    ...
    useEffect(() => {
        ...
        const updatePhysics = (dt: number) => {
            ...
            if (currentState.gameState !== GameState.RUNNING) return;
        ...
        const draw = (time: number) => {
            ...
            // --- BESPOKE BIOME BACKGROUND LAYER ---
            // We implement a "cover" aspect-ratio scaling algorithm to ensure the rainforest.png
            // spans the entire viewport without distortion. The "almost invisible" opacity 
            // preserves the visual hierarchy and contrast for the neon assets.
            if (bgImageRef.current && bgImageRef.current.complete) {
                ctx.save();

                // Dynamic opacity: slightly more visible during planning.
                ctx.globalAlpha = currentState.gameState === GameState.PLANNING ? 0.05 : 0.02;

                // 'screen' composite subtly lightens the dark void, blending the jungle texture
                // into the neon sci-fi aesthetic without causing muddy pixel wash.
            ...
            // --- STANDARD MODE VISUALS ---
                ...
                // ═══════════════════════════════════════════════════════════
                // BACKGROUND AMBIENCE (AURORAS, NEBULAS, SHOOTING STARS)
                // ═══════════════════════════════════════════════════════════
                const biome = (settings.currentLevel || 0) % 4;
                ...
                if (true) {
                    ...
                    switch (biome) {
                        case 0: { // 🌿 BIOME 0: BOREAL AURORA
                            ...
                            const auroraBands = [
                                ...
                                { color: [120, 40, 255], yBase: 0.35, freq: 0.00038, amp: 0.10, speed: 0.00023, opacity: 0.09 },
                                { color: [0, 255, 140], yBase: 0.18, freq: 0.00055, amp: 0.06, speed: 0.00014, opacity: 0.11 },
                                { color: [180, 0, 255], yBase: 0.45, freq: 0.00030, amp: 0.12, speed: 0.00027, opacity: 0.08 },
                            ];

                            auroraBands.forEach(band => {
                                const bandH = height * 0.35; // Much wider spread
                                const waveY = (x: number) =>
                                    band.yBase * height +
                                    Math.sin(x * band.freq + time * band.speed) * band.amp * height +
                                    Math.sin(x * band.freq * 1.7 + time * band.speed * 0.6 + 1.2) * band.amp * height * 0.4;

                                const midY = waveY(width / 2);
                                const grad = ctx.createLinearGradient(0, midY - bandH * 0.6, 0, midY + bandH * 0.6);
                                const [r, g, b] = band.color;
                                grad.addColorStop(0, `rgba(${r},${g},${b},0)`);
                                grad.addColorStop(0.3, `rgba(${r},${g},${b},${band.opacity * 0.5})`);
                                grad.addColorStop(0.5, `rgba(${r},${g},${b},${band.opacity})`);
                                grad.addColorStop(0.7, `rgba(${r},${g},${b},${band.opacity * 0.5})`);
                                grad.addColorStop(1, `rgba(${r},${g},${b},0)`);

                                ctx.fillStyle = grad;
                                ctx.beginPath();
                                ctx.moveTo(0, waveY(0) - bandH * 0.6);
                                for (let sx = 0; sx <= width; sx += 8) ctx.lineTo(sx, waveY(sx) - bandH * 0.6);
                                for (let sx = width; sx >= 0; sx -= 8) ctx.lineTo(sx, waveY(sx) + bandH * 0.6);
                                ctx.closePath();
                                ctx.fill();
                            });

                            // Bright, expansive nebulae cores
                            const nebulae = [
                                { cx: 0.20, cy: 0.30, rx: 0.50, ry: 0.30, color: [0, 120, 255], driftX: 0.00006, driftY: 0.00004, opacity: 0.06 },
                                { cx: 0.80, cy: 0.70, rx: 0.60, ry: 0.35, color: [140, 0, 255], driftX: -0.00005, driftY: -0.00003, opacity: 0.05 },
                                { cx: 0.50, cy: 0.85, rx: 0.45, ry: 0.30, color: [0, 200, 180], driftX: 0.00004, driftY: 0.00007, opacity: 0.04 },
                            ];
                            nebulae.forEach(n => {
                                const cx = (n.cx + Math.sin(time * n.driftX) * 0.06) * width;
                                const cy = (n.cy + Math.cos(time * n.driftY) * 0.05) * height;
                                const rx = n.rx * width;
                        ...
                        case 1: { // 🌊 BIOME 1: ABYSSAL CURRENTS
                            // Massive, sweeping deep-sea light shafts
                            const numRays = 6;
                            for (let i = 0; i < numRays; i++) {
                                const baseX = (i / numRays) * width;
                                const swing = Math.sin(time * 0.00015 + i) * width * 0.15;
                                const x = baseX + swing;

                                const rayW = width * 0.25; // Wider shafts
                                const grad = ctx.createLinearGradient(x - rayW / 2, 0, x + rayW / 2, 0);
                                grad.addColorStop(0, 'rgba(0,180,255,0)');
                                grad.addColorStop(0.5, 'rgba(0,255,200,0.06)'); // Much brighter
                                ...
                                ctx.lineTo(x + rayW / 2 + Math.cos(time * 0.0001 + i) * 150, height);
                                ctx.lineTo(x - rayW / 2 + Math.cos(time * 0.0001 + i) * 150, height);
                                ctx.closePath();
                                ctx.fill();
                            }

                            // Glowing bioluminescent plankton drifting upwards
                            ctx.fillStyle = 'rgba(0, 255, 180, 0.4)';
                            for (let i = 0; i < 40; i++) {
                                const px = ((i * 137.5 + time * 0.01) % width + width) % width;
                                const py = height - (((i * 91.3 + time * 0.02) % height + height) % height);
                                const pSize = 1 + Math.sin(time * 0.001 + i) * 1.5;
                                if (pSize > 0) {
                                    ctx.beginPath();
                        ...
                        case 2: { // 🌅 BIOME 2: ZEN RADIANCE
                            // Sweeping, highly visible god-rays
                            const pivotX = width * 0.5 + Math.sin(time * 0.0001) * width * 0.3;
                            ...
                            for (let i = 0; i < numSunRays; i++) {
                                const angle = Math.PI * 0.3 + (i * 0.15) + Math.sin(time * 0.00005 + i) * 0.1;
                                ctx.rotate(angle);

                                const rayLen = width * 2.5;
                                const rayThick = 300 + Math.sin(time * 0.0001 + i) * 150;

                                const sGrad = ctx.createLinearGradient(0, -rayThick / 2, 0, rayThick / 2);
                                sGrad.addColorStop(0, 'rgba(255,140,0,0)');
                                sGrad.addColorStop(0.5, 'rgba(255,180,50,0.08)'); // High visibility warm gold
                                sGrad.addColorStop(1, 'rgba(255,140,0,0)');

                                ctx.fillStyle = sGrad;
                                ctx.fillRect(0, -rayThick / 2, rayLen, rayThick);
                                ctx.rotate(-angle);
                            }
                            ctx.restore();

                            // Ambient core warmth
                            const warmNebula = ctx.createRadialGradient(width * 0.5, height * 0.3, 0, width * 0.5, height * 0.3, width * 0.8);
                            warmNebula.addColorStop(0, 'rgba(255, 120, 0, 0.08)');
                            warmNebula.addColorStop(1, 'rgba(0,0,0,0)');
                            ctx.fillStyle = warmNebula;
                            ctx.fillRect(0, 0, width, height);

                            // Procedural dust motes floating in the light
                            for (let i = 0; i < 50; i++) {
                                const dx = ((i * 314 + Math.sin(time * 0.0005 + i) * 100) % width + width) % width;
                                const dy = ((i * 271 + Math.cos(time * 0.0003 + i) * 100) % height + height) % height;
                                const alpha = (Math.sin(time * 0.001 + i) + 1) * 0.5 * 0.6; // 0 to 0.6 opacity
                        ...
                        case 3: { // 💠 BIOME 3: SACRED GEOMETRY
                            ctx.save();
                            ctx.translate(width / 2, height / 2);

                            // Deep void core pulse
                            const basePulse = (Math.sin(time * 0.0005) + 1) * 0.5; // 0 to 1
                            const coreGlow = ctx.createRadialGradient(0, 0, 0, 0, 0, width * 0.6);
                            coreGlow.addColorStop(0, `rgba(188, 19, 254, 0.12)`);
                            ...
                            // High-impact wireframes
                            const drawPoly = (sides: number, radius: number, angleOffset: number, color: string, glowColor: string) => {
                            ...
                            // Heavy, thick neon rings
                            drawPoly(6, width * 0.3 + basePulse * 30, time * 0.0001, `rgba(188, 19, 254, 0.2)`, '#bc13fe');
                            drawPoly(6, width * 0.45 - basePulse * 20, -time * 0.00015, `rgba(0, 243, 255, 0.15)`, '#00f3ff');
                            drawPoly(12, width * 0.6, time * 0.00005, `rgba(255, 0, 255, 0.1)`, '#ff00ff');

                            // Connecting laser struts
                            ctx.beginPath();
                            for (let i = 0; i < 6; i++) {
                                const a = (i / 6) * Math.PI * 2 + time * 0.0001;
                                ctx.moveTo(Math.cos(a) * width * 0.3, Math.sin(a) * width * 0.3);
                                ctx.lineTo(Math.cos(a) * width * 1.5, Math.sin(a) * width * 1.5);
                            }
                    ...
                    // ═══════════════════════════════════════════════════════════
                    // SHOOTING STARS (Only for Biome 0 and 3)
                    // ═══════════════════════════════════════════════════════════
                    if (biome === 0 || biome === 3) {
                        if (time >= engineRef.current.nextShootingStar) {
                            // More frequent, cinematic meteors
                            engineRef.current.nextShootingStar = time + 1000 + Math.random() * 2500;
                            const angle = (Math.random() * 0.4 + 0.1) * Math.PI;
                            const speed = 12 + Math.random() * 10;
                            engineRef.current.shootingStars.push({
                                x: Math.random() * width,
                                y: Math.random() * height * 0.5,
                                vx: Math.cos(angle) * speed,
                                vy: Math.sin(angle) * speed,
                                length: 150 + Math.random() * 200, // Longer tails
                                alpha: 0.9 + Math.random() * 0.3, // Brighter
                        ...
                        ctx.save();
                        ctx.globalCompositeOperation = 'screen';
                        engineRef.current.shootingStars = engineRef.current.shootingStars.filter(s => s.active);
                        engineRef.current.shootingStars.forEach(s => {
                            s.x += s.vx;
                            s.y += s.vy;
                            s.alpha -= s.decay;
                            if (s.alpha <= 0 || s.x > width + 50 || s.y > height + 50) {
                                s.active = false;
                                return;
                            }
                            ...
                            // Vibrant neon comet heads
                            const glowColor = biome === 3 ? `rgba(255,50,255,${s.alpha * 0.8})` : `rgba(100,255,255,${s.alpha * 0.8})`;
                            sGrad.addColorStop(0.8, glowColor);

                            sGrad.addColorStop(1, `rgba(255,255,255,${s.alpha})`);
                            ...
                            const flare = ctx.createRadialGradient(s.x, s.y, 0, s.x, s.y, 8);
                            flare.addColorStop(0, `rgba(255,255,255,${s.alpha})`);
